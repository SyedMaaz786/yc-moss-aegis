...
const secrets = ['MOSS_PROJECT_KEY','GROQ_API_KEY','VERCEL_TOKEN'].map(k=>env[k]).filter(v=>v&&v.length>12);
...
for(const file of files) {
 ...
 if(secrets.some(secret=>content.includes(secret)) || /\b(?:gsk_[A-Za-z0-9]{25,}|gh[pousr]_[A-Za-z0-9]{30,})\b/.test(content)) findings.push(file);
}
